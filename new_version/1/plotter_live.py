...
import live_plotter_only_puls

exit_flag = False
checkbox_on_flag = False
data_to_plot_IR = []
...
class PlotterWindow(baseclass, uiclass):
    def __init__(self, df):
        ...
        self.actionNur_Puls.triggered.connect(self.nur_pulse_klicked)
            ...
            if current_filter_IR == "no_filter":
                ...
                async def test_async():
                    ...
                    while self.checkbox_state_ir == 2:
                        ...
                        if data_to_plot_IR: print("Daten zum Plotten:", data_to_plot_IR)
                        self.update_plot_data_no_filter()
    ...
    def update_plot_data_no_filter(self):
        ...
        for data_point_IR in data_to_plot_IR:  #data_point: Diese Variable repräsentiert jeden einzelnen Datenpunkt, der während der Iteration durch data_to_plot geplottet wird.
            ...
            # np.roll ist eine Funktion von NumPy, die die Elemente eines Arrays zyklisch verschiebt. self.data wird um eins nach links verschoben, wodurch der letzte Wert 
            # entfernt und Platz für den neuen Wert am Ende geschaffen wird.

            self.data = np.roll(self.data, -1) 
            self.data[-1] = data_point_IR #Setzt den letzten Wert von self.data auf data_point_IR, also den aktuellen Datenpunkt.
            ...
            time.sleep(0.020)  # Pause von 10 Millisekunden
    ...
    def einstellungen_klicked(self):
        ...
        # Funktion die mit dem Button live plotter puls verbunden ist und das Fenster öffnet

    def nur_pulse_klicked(self):
        global pulse_window
        print("Nur Pulse gedrückt")
        from live_plotter_only_puls import LivePlotterPulsWindow

        pulse_window = LivePlotterPulsWindow()
        ...
        from safe_to_excel import df
        pulse_window.plot(df)  # Aufruf der plot-Funktion, um den Graphen anzuzeigen
        pulse_window.show()  # Anzeigen des Plotter Fensters
...
class DataUpdater(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            if not checkbox_on_flag: # Wenn die Checkbox Flag nicht on ist, d.h. keine Checkbox on ist, wird die Pufferliste gelöscht
                self.list_data_ir_puffer.clear()
