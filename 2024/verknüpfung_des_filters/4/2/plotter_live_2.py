...
class PlotterWindow(baseclass, uiclass):
    ...
    def update_plot_data_no_filter(self):
        ...
        self.index = 0  # Startindex für den Datenabruf

        self.plot_timer = QTimer()
        self.plot_timer.timeout.connect(self.plot_next_data_point)
        self.plot_timer.start(17)  # Alle 17 Millisekunden aktualisieren


    def plot_next_data_point(self):
        if self.index < len(self.data_to_plot_IR):
            data_point_IR = self.data_to_plot_IR[self.index]
            print(f"Datenpunkt {self.index + 1}: {data_point_IR}")
            ...
            self.plot_timer.stop()  # Stoppen des Timers, wenn alle Datenpunkte geplottet wurden
            self.data_updater.list_data_ir_puffer = self.data_updater.list_data_ir_puffer[SAMPE_ARRAY:]  # Entfernen der geplotteten Datenpunkte
