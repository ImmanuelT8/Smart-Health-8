...
import asyncio
from PyQt6.QtWidgets import QApplication  # Import von QApplication hinzugefügt
from PyQt6.QtCore import QEventLoop  # Import von QEventLoop hinzugefügt
...
class PlotterWindow(baseclass, uiclass):
    def __init__(self, df, tg):
        ...
        self._tg = tg  # Hier wird die TaskGroup gespeichert
        # Verbindung zur Methode update_plot_no_filter() herstellen
        einstellungs_fenster.checkbox_ir_state_changed.connect(lambda: self._tg.create_task(self.update_plot_no_filter()))
    ...
    async def checkbox_ir_state_changed(self, state):
        # Hier können Sie den Wert 'state' verwenden, der von der Checkbox im Einstellungsfenster übergeben wurde
        print("IR Checkbox Zustand geändert:", state)
        self.checkbox_state_ir = state
        ...
        if state:  # Wenn die Checkbox aktiviert ist
            ...
            if current_filter_IR == "no_filter":
                ...
                await self.update_plot_data_no_filter()


            if current_filter_IR == "mean_filter":

                await self.update_plot_data_mean_filter()



    def checkbox_ir_state_changed(self, state):
        global current_filter_IR
        print("Checkbox IR aktiviert")
    ...
    async def update_plot_data_no_filter(self):
        global SAMPE_ARRAY
...
class DataUpdater(QThread):
    ...
    def stop(self):
        self.running = False
        print("Data Updater Thread geschlossen")
...
def main():
    ...
    async def amain():
        async with asyncio.TaskGroup() as tg:
            main_window = PlotterWindow(tg)
            main_window.show()
            await app_close_event.wait()

    asyncio.run(amain(), loop_factory=lambda: event_loop)
