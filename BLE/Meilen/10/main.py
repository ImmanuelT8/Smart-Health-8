...
class PruefeDateinameThread(QThread):
    ...
    async def pruefe_dateiname(self, Dateiname):
        ...
        if Dateiname == 0:
            print("Starte Thread im Mainfile in pruefe Dateiname first run")
            safe_to_excel.start_threads() 
        ...
        if restart:
            safe_to_excel.start_threads() 
            print("Starte Thread im Mainfile in pruefe Dateiname second rund and more..")
...
class StoppThread (QThread):
    ...
    def run(self):
        ...
        async def stop_tasks():
            ...
            await asyncio.sleep(5)  # Pause für 5 Sekunden
            Start.setEnabled(True)
...
async def pruefe_dateiname(Dateiname):
    global restart
    if Dateiname == 0: # Wenn der Dateiname nicht 0 ist (erster Durchlauf) werden die Start Threads im safe to excel gestartet
        safe_to_excel.start_threads()
    ...
    if restart: #Alternativ wenn bereits einmal auf Stopp geklickt wurde werden die Threads im safe to excel ebenfalls gestartet
        print("Starte Thread im pruefe Dateiname durch restart Variable")
        safe_to_excel.start_threads()
        restart = False
    ...
    if restart:
            safe_to_excel.start_threads()
            print("Starte Thread bei restart")
...
def Update_clicked():
    ...
    # Setzt die Textfarbe auf Schwarz, wenn das Einstellungen-Fenster geöffnet wird
    einstellungen_window_ui.Parameter_Update_Text.setStyleSheet("color: black;")

    Variable1 = int(einstellungen_window_ui.updateLed.toPlainText())
    Variable2 = einstellungen_window_ui.updateProbenzahl.toPlainText()
    Variable3 = einstellungen_window_ui.updateLedmodus.toPlainText()
    Variable4 = einstellungen_window_ui.updateSamplerate.toPlainText()
    Variable5 = einstellungen_window_ui.updatePulswidth.toPlainText()
    Variable6 = einstellungen_window_ui.updateAdc.toPlainText()
